# 360 PhotoLab
# Public release: v0.37.0
# SPDX-License-Identifier: GPL-3.0-or-later
#
# Based on internal script: reframe360_v033_complete.py

# ============================================================
# VERSIONE: 037 | 2026-05-16 08:40 (Europe/Rome)
# MODIFICA:
# - Versione pubblica aggiornata a 0.37.0 / V037.
# - Interfaccia/menu mantenuti in italiano.
# - Nessuna modifica alla logica funzionale.
# ============================================================

bl_info = {
    "name": "360 PhotoLab V037",
    "author": "Mattia Fiorini",
    "version": (0, 37, 0),
    "blender": (4, 2, 0),
    "location": "View3D > Sidebar > 360 PhotoLab",
    "description": "Riframma foto 360 equirettangolari con vista camera, anteprime CIL/CLV, output rettilineare, LUT ed esportazione PNG/TIFF/JPEG",
    "category": "Camera",
}
...
CAMERA_NAME = "Reframe360_Camera_V036"
WORLD_NAME = "Reframe360_World_V036"
PREVIEW_CAMERA_NAME = "Reframe360_Preview_Camera_V036"
PREVIEW_COLLECTION_NAME = "Reframe360_Preview_V036"
CIL_GUIDE_OBJECT_NAME = "Reframe360_CIL_Guide_Rect_V036"
CIL_GUIDE_LABEL_NAME = "Reframe360_CIL_Guide_Label_V036"
CIL_GUIDE_MATERIAL_NAME = "Reframe360_CIL_Guide_Mat_V036"
...
def export_format_technical_note(fmt):
    if fmt == "TIFF16":
        return "Senza perdita, RGB 16 bit: master consigliato per stampa/modifica."
    if fmt == "JPEG":
        return "Con perdita: copia leggera per invio/condivisione, non master."
    return "Senza perdita, RGBA 8 bit: default sicuro e compatibile."
...
def update_camera(scene):
    ...
    # V036: la Camera View deve restare stabile quando si cambia RET/CIL/CLV.
...
def setup_world_image(scene, filepath):
    ...
    env.name = "Foto 360 Equirettangolare V036"
    env.image = img
...
def build_cilindrical_boundary_points_for_camera(scene, cam):
    ...
    V036: il calcolo usa il centro reale del frame camera restituito da Blender.
...
def write_tiff_rgb16_uncompressed(filepath, rgba, np, dpi=300):
    ...
    software = b"Reframe360 V036\x00"
...
def write_jpeg_via_blender(filepath, rgba, np, quality=95):
    ...
    img = bpy.data.images.new("Reframe360_Output_JPEG_TEMP_V036", width=w, height=h, alpha=True, float_buffer=False)
...
def estimate_source_density_resolution(scene):
    ...
    Metodo V036: campiona il quadro corrente, lo rimappa sulla foto equirettangolare,
...
class REFRAME360_OT_load_lut(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        self.report({"INFO"}, "LUT caricata: sarà applicata sempre ad anteprime ed esportazioni")
        return {"FINISHED"}
...
class REFRAME360_OT_orbit_view(Operator):
    ...
    bl_label = "Orbita / Inquadra"
    bl_description = "Trascina il mouse per orientare la vista. Esc/tasto destro annulla, Invio/clic sinistro conferma. Rotella = FOV."
    ...
    def invoke(self, context, event):
        ...
        self.report({"INFO"}, "Orbita: trascina. Shift=fine, Ctrl=rapido, rotella=FOV, Esc=annulla.")
    ...
    def modal(self, context, event):
        ...
        if event.type in {"ESC", "RIGHTMOUSE"}:
            ...
            self.report({"INFO"}, "Orbita annullata")
            return {"CANCELLED"}

        if event.type in {"RET", "NUMPAD_ENTER", "LEFTMOUSE"} and event.value == "PRESS":
            self.report({"INFO"}, "Orbita confermata")
            return {"FINISHED"}
...
class REFRAME360_OT_open_camera_view(Operator):
    ...
    bl_label = "Vista camera renderizzata"
...
class REFRAME360_OT_render_save(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({"ERROR"}, f"Errore esportazione: {e}")
            return {"CANCELLED"}

        self.report(
            {"INFO"},
            f"Esportazione salvata: {saved_path}",
        )
...
class REFRAME360_PT_panel(Panel):
    bl_label = "360 PhotoLab V037"
    bl_idname = "REFRAME360_PT_panel"
    bl_space_type = "VIEW_3D"
    bl_region_type = "UI"
    bl_category = "360 PhotoLab"
    ...
    def draw(self, context):
        ...
        box.operator("reframe360.orbit_view", text="ORBITA / INQUADRA")
        ...
        box.separator()
        box.label(text="Stampa teorica esportazione corrente:")
        ...
        box.operator("reframe360.render_save", text=f"Esporta {export_format_short_label(s.reframe360v33_export_format)} + salva")
...
def register_props():
    ...
    bpy.types.Scene.reframe360v33_export_format = EnumProperty(
        name="Formato esportazione",
        items=[
            ("PNG8", "PNG 8 bit senza perdita", "Formato default: senza perdita, molto compatibile"),
            ("TIFF16", "TIFF 16 bit senza perdita", "Master per stampa/modifica: senza perdita, non compresso"),
            ("JPEG", "JPEG qualità alta", "Formato con perdita per file leggeri e condivisione"),
        ],
        default="PNG8",
    )
