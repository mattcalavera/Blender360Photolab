...
**360 PhotoLab** is a beta add-on for Blender that reframes **equirectangular 360° still photos** into conventional photographic images.

It is designed for drone, panoramic and 360° photo workflows where you want to extract a controlled still image from a full equirectangular panorama while choosing projection, field of view, framing, LUT correction and export resolution directly inside Blender.

> **Status:** beta. The add-on is usable, but the interface and internal projection tools are still evolving.

360 PhotoLab is an independent project and is not affiliated with, endorsed by or sponsored by the Blender Foundation.

![360 PhotoLab camera workflow](/images/Main-camera-view.png)
...
- Comparative preview layout:
  ...
  - **RET** centered below.
- Preview view automatically switches to an orthographic layout, centered on the previews, without the semi-transparent camera frame.
- `.cube` LUT support.
...
### Add-on panel and export tools

![Add-on panel 1](/images/addon-panel_1.png)

![Add-on panel 2](/images/addon-panel_2.png)

---

## What it is for

360 PhotoLab is useful when you have a high-resolution 360° still image and want to create a normal-looking photographic export, for example:
...
In this mode, pitch is treated more like a **vertical framing shift** rather than a full camera tilt inside the cylindrical projection. This is intended to reduce visible horizon curvature in many panoramic reframes.
...
The add-on appears in the Blender 3D View sidebar under the **360 PhotoLab** tab.
...
For Blender 4.2+ and 5.x, install 360 PhotoLab using the **extension ZIP attached as a release asset**.

Do **not** install GitHub's automatic **Source code (zip)** archive. That archive is generated by GitHub and is not the packaged extension ZIP.
...
8. Open the 3D View sidebar and look for the **360 PhotoLab** tab.

The extension package must include a `blender_manifest.toml` file. This file is part of Blender's extension packaging system and is expected in the release ZIP.
...
4. Go to **360 PhotoLab**.
5. Click **Load 360 Photo** / **Carica foto 360** and select an equirectangular image.
6. 360 PhotoLab automatically switches to Rendered Camera View.
...
360 PhotoLab is released under:
